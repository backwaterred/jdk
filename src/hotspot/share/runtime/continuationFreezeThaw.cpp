...
inline freeze_result FreezeBase::recurse_freeze_java_frame(const frame& f, frame& caller, int fsize, int argsize) {
  ...
  assert(fsize > 0, "");
  assert(argsize >= 0, "Expected non-negative argsize: %", argsize);
  _freeze_size += fsize;
  NOT_PRODUCT(_frames++;)
...
freeze_result FreezeBase::recurse_freeze_compiled_frame(frame& f, frame& caller,
  ...
  const int argsize = ContinuationHelper::CompiledFrame::stack_argsize(f) + frame::metadata_words_at_top AIX_ONLY(- !callee_interpreted ?  8 : 0);
